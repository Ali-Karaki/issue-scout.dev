      ...
      <AnimatedSection delay={0}>
        <section className="max-w-4xl mx-auto px-6 pt-24 pb-20 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            <Link
              href="/issues"
              className="px-4 py-2 rounded-lg bg-amber-500 text-black font-medium hover:bg-amber-400 transition-colors duration-200 no-underline"
            >
              Browse issues
            </Link>
              ...
              {PROJECTS.map((proj) => (
                <AnimatedItem key={proj.id}>
                  <Link
                    ...
                    className="px-4 py-2 rounded-lg bg-zinc-800/80 text-zinc-300 hover:bg-zinc-700 hover:text-white no-underline text-sm transition-colors duration-200"
                  >
                    {proj.name}
                  </Link>
                </AnimatedItem>
